...
a <- ggplot(total.extinction.sum, aes(ymin=total.extinction.05*100, ymax=total.extinction.95*100, x=O2.pal*100))+
  ...
  annotate(geom="text", x=.32*100, y=99, label="A) Pre-industrial baseline", fontface="italic", size=10.3)
...
b <- ggplot(total.extinction.sum, aes(ymin=total.extinction.05*100, ymax=total.extinction.95*100, x=O2.pal*100))+
  annotate(geom="rect", xmin=.1*100, xmax=.5*100, ymin=-Inf, ymax=Inf, fill="grey85")+
  annotate(geom="text", x=.3*100, y=-1, label = expression("Early Paleozoic O"[2]), color="grey0", size=7)+
  geom_ribbon(aes(group=Tectonics), alpha=1, fill="grey100")+ 
  geom_ribbon(aes(group=Tectonics, fill=Tectonics),  size=.1, color="grey70",  alpha=.4)+ 
  ...
  theme(plot.margin = margin(.5,1,1,1,"cm"),panel.border = element_rect(fill=NA,color="black", size=2,linetype="solid"),
        ...
        panel.grid.major = element_blank(),panel.grid.minor = element_blank())+coord_cartesian(xlim=c(0,100), ylim=c(0,100))+
  guides(shape = guide_legend(order = 2), fill = guide_legend(order = 1))+
  annotate(geom="text", x=.21*100, y=99, label="B) Cool Baseline", fontface="italic", size=10.3)
...
c <- ggplot(total.extinction.sum, aes(ymin=total.extinction.05*100, ymax=total.extinction.95*100, x=O2.pal*100))+
  annotate(geom="rect", xmin=.1*100, xmax=.5*100, ymin=-Inf, ymax=Inf, fill="grey85")+
  annotate(geom="text", x=.3*100, y=-1, label = expression("Early Paleozoic O"[2]), color="grey0", size=7)+
  geom_ribbon(aes(group=Tectonics), alpha=1, fill="grey100")+ 
  geom_ribbon(aes(group=Tectonics, fill=Tectonics),  size=.1, color="grey70",  alpha=.4)+ 
  ...
  theme(plot.margin = margin(.5,1,1,1,"cm"),panel.border = element_rect(fill=NA,color="black", size=2,linetype="solid"),
        ...
        panel.grid.major = element_blank(),panel.grid.minor = element_blank())+coord_cartesian(xlim=c(0,100), ylim=c(0,100))+
  guides(shape = guide_legend(order = 2), fill = guide_legend(order = 1))+
  annotate(geom="text", x=.22*100, y=99, label="C) Warm Baseline", fontface="italic", size=10.3)

plots <- ggarrange2(a,b,c, ncol=2)

ggsave("Figures/Extinction figure climate state 1EFD shelves (Figure S8).pdf", plots, width=20, height=18, units="in")
